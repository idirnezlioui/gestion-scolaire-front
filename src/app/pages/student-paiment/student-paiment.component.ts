...
export class StudentPaimentComponent {
  ...
  constructor(private etudiantService:StudentService , private paiementService:PaiementService){}
  ...
  //recherche par id 
  searchById(){
    this.filterStudents=this.students.filter(student=>student.num_etudiant.toString().includes(this.searchId))
  }

  //recherche par nom et prenom

  searchByName(){
    this.filterStudents=this.students.filter(student=>(student.nom+' '+student.prenom).toLowerCase().includes(this.searchterm.toLowerCase()))
  }
